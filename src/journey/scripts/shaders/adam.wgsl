    ...
    *mem.t += 1u;
    for(var i = 0u; i < DIM_GRAD; i++) {
        ...
        *mem.v[i] = p.b2 * (*mem.v[i]) + (1.0 - p.b2) * gi * gi;// raw second momentum estimate
        let mHat = *mem.m[i] / (1.0 - pow(p.b1,*mem.t));        // bias corrected first moment
        let vHat = *mem.v[i] / (1.0 - pow(p.b2,*mem.t));        // bias corrected second moment
