...
the goal output we want our function $f$ to have in this example is:
...
We start with a random guess for our parameter $\mu_0 \in [-5, 5]$. Our goal now is to find the optimal $\mu^{*}$ that approximates our $y_i$ the best, this can be done by minimizing our loss function $L$, so our new goal is:
...
The implementation in the compute shader could look like this:
...
```
You can try executing the compute shader yourself, and experiment with the parameters bellow:
